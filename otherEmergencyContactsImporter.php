...
require_once(__DIR__ . '/PhoneNumberFilter.php');
...
class OtherEmergencyContactsImporter extends Model_Import_Base {
     ...
     * and returns array with both person formed name
     */
    protected function filterNames($input)
    {
        $validNames = array();

        // Define all delimiters for names
        $namesSeparator = array(
            '&' => '',
            ' AND ' => '',
            '/' => '',
            '\\' => '',
        ...
        $input = preg_replace('/\s+/', ' ', strtoupper($input));
        $names = array($input);
        foreach ($namesSeparator as $delimiter => $replace) {
            if (false !== strpos($input, $delimiter)) {
                $names = explode($delimiter, $input);
                break;
            }
        }

        $names = array_unique($names);
        // Trims all white characters within name
        array_walk($names, function (&$val) {
            $val = trim($val);
        });

        $namesCount = count($names);
        // Assigns Name to name1 if there is no delimiter from the pattern array
        if ($namesCount == 1) {
            $validNames[] = $names[0];
        }
        // If there is more than one name, takes second name and assigns value to name2 and adds surname to name1
        if ($namesCount == 2) {
            $name2 = $names[1];
            $names2 = explode(" ", $name2);
            array_shift($names2);
            $validNames[] = $names[0] . ' ' . implode(' ', $names2);
            $validNames[] = $name2;
        }
...
//                if($rowNumber >= 400) {
                $this->processRow($rowNumber, $row, 4);
//                }
            } else echo "Skipping row :" . $rowNumber . " because it is less than startRow: $startRow\n";
        }
    ...
    public function processRow($rowNumber, $row, $emergencyContactPriority) {

//        $input = 'Margaret Cleator Grandma 817791/479170w/978444/B Harvey 628287/481674';
        if($row['Contacts (other)']) {
            $input = $row['Contacts (other)'];
            preg_match_all('/(\D{3,})([\d\s\/who]+)/si', $input, $matches);
...
include(__DIR__."/import-helper.php");


$env = [
    "targetApiEndpoint"=>$targetApiEndpoint
];

$fileName = $inputFile;

$importer = new OtherEmergencyContactsImporter($env);
$importer->processFile($fileName, $startRow);
